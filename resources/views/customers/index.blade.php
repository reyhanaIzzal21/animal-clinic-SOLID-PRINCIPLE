<x-app>
    <div class="min-h-screen bg-gray-50">
        <div class="max-w-7xl mx-auto p-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
                <h1 class="text-2xl font-semibold text-[#FBA518]">Daftar Customers</h1>
                    ...
                    <table class="w-full">
                        <thead class="bg-[#FBA518] text-white">
                            <tr>
                                ...
                                <th class="py-4 px-6 text-left hidden md:table-cell">Alamat</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach ($customers as $customer)
                                <tr class="hover:bg-gray-50 transition-all duration-200">
                                    ...
                                    <td class="py-4 px-6 hidden md:table-cell">{{ $customer->address }}</td>
                                    <td class="py-4 px-6 hidden lg:table-cell">{{ $customer->email }}</td>
                                    <td class="py-4 px-6 hidden sm:table-cell">{{ $customer->phone }}</td>
                                    <td class="py-4 px-6">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <button type="button" data-modal-target="deleteModal-{{ $customer->id }}"
                                                data-modal-toggle="deleteModal-{{ $customer->id }}"
    ...
    <!-- Delete modal -->
    @foreach ($customers as $customer)
        <div id="deleteModal-{{ $customer->id }}" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                ...
                <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-50 sm:p-5">
                    ...
                    <div class="flex justify-center items-center space-x-4">
                        <button data-modal-toggle="deleteModal-{{ $customer->id }}" type="button"
                        ...
                        <form action="{{ route('customers.destroy', $customer->id) }}" method="POST"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="p-2 bg-[#E52020] text-white rounded-2xl hover:bg-red-700 transition-all duration-300">
                                hapus
                            </button>
                        </form>
                    </div>
