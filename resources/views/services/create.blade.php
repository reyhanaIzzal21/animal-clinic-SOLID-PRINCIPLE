<x-app>
    <div class="max-w-lg mx-auto p-6 bg-white shadow-xl rounded-lg">
        <h2 class="text-2xl font-semibold text-[#FBA518] mb-6 text-center">Tambah Service</h2>
        <form action="{{ route('services.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description"
                    class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FBA518]"></textarea>
                @error('description')
                    <p class="text-red-500 italic text-xs">
                        {{ $message }}
                    </p>
